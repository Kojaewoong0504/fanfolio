...
function App() {
  ...
  if (showOnboarding) {
    return <Onboarding userId={currentUser?.id ?? 'fan'} onComplete={() => { setShowOnboarding(false); void refreshUser(); void refreshCollection() }} onBack={logout} />
  }
      ...
      <header className="app-header">
        ...
        <div className="header-actions">
          ...
          <button className="header-profile-button" onClick={() => navigateTab('settings')} aria-label="프로필 및 설정">
            <span>{(currentUser?.nickname ?? '팬').slice(0, 1)}</span>
          </button>
        </div>
      </header>
...
function Login({ onLogin }: { onLogin: () => void | Promise<void> }) {
  ...
  return <main className="login-screen"><span className="brand-mark">F</span><p className="eyebrow">FANFOLIO</p><h1>내 손안의<br />팬 컬렉션</h1><p className="muted">좋아하는 아티스트의 순간을<br />디지털 카드로 간직하세요.</p><div className="social-login"><button className="social-button kakao" onClick={() => { window.location.href = oauthStartUrl('kakao') }} disabled={busy}><span aria-hidden="true">K</span>카카오로 계속하기</button><button className="social-button google" onClick={() => { window.location.href = oauthStartUrl('google') }} disabled={busy}><span aria-hidden="true">G</span>Google로 계속하기</button></div><div className="login-divider"><span>또는 이메일로 로그인</span></div>{!requested && <div className="auth-mode" role="tablist" aria-label="인증 방식"><button className={purpose === 'login' ? 'active' : ''} role="tab" aria-selected={purpose === 'login'} onClick={() => setPurpose('login')}>로그인</button><button className={purpose === 'signup' ? 'active' : ''} role="tab" aria-selected={purpose === 'signup'} onClick={() => setPurpose('signup')}>회원가입</button></div>}<label className="field-label" htmlFor="login-email">이메일</label><input id="login-email" value={email} onChange={e => setEmail(e.target.value)} placeholder="이메일을 입력하세요" type="email" disabled={requested} />{!requested ? <button className="primary" onClick={() => void requestLink()} disabled={!email.includes('@') || busy}>{busy ? '보내는 중...' : purpose === 'signup' ? '회원가입 링크 받기' : '로그인 링크 받기'}</button> : <><label className="field-label" htmlFor="login-token">인증 토큰</label><input id="login-token" value={token} onChange={e => setToken(e.target.value)} placeholder="이메일의 인증 토큰을 입력하세요" /><button className="primary" onClick={() => void verifyLink()} disabled={!token || busy}>{busy ? '확인 중...' : purpose === 'signup' ? '회원가입하기' : '로그인하기'}</button></>}<p role={messageIsError ? 'alert' : 'status'} className={messageIsError ? 'form-message error-message' : 'form-message'}>{message}</p><p className="login-note">소셜 로그인은 빠르게 시작할 수 있고, 이메일 로그인도 계속 사용할 수 있어요.</p></main>
}

function Onboarding({ userId, onComplete, onBack }: { userId: string, onComplete: () => void, onBack: () => Promise<void> }) {
  ...
  return <main className="onboarding-screen">
    ...
    {step === 3 && <><p className="eyebrow">나만의 컬렉션</p><h1>팬포리오에서 사용할<br />닉네임을 정해 주세요</h1><p className="muted">{selectedArtist?.name ?? '좋아하는 아티스트'} · {members.find(item => item.id === member)?.name ?? '선택한 멤버'}의 카드를 모아볼게요.</p><div className="nickname-preview" aria-live="polite"><div className="nickname-avatar">{nickname.trim().slice(0, 1) || '팬'}</div><div><span>컬렉션 프로필</span><b>{nickname.trim() || '나의 팬 닉네임'}</b><small>{selectedArtist?.name ?? '좋아하는 아티스트'} · {members.find(item => item.id === member)?.name ?? '선택한 멤버'}</small></div></div><label className="field-label" htmlFor="onboarding-nickname">닉네임</label><div className="nickname-input-wrap"><input id="onboarding-nickname" className="nickname-input" value={nickname} onChange={e => setNickname(e.target.value)} placeholder="닉네임을 입력하세요" maxLength={40} aria-describedby="nickname-help" /><small>{nickname.length}/40</small></div><p id="nickname-help" className="field-help">나중에 설정에서 언제든 바꿀 수 있어요.</p><button type="button" className="primary" onClick={() => void save()} disabled={!nickname.trim() || busy || backBusy}>{busy ? '저장 중...' : '나만의 컬렉션 시작하기'}</button></>}
