...
run_code "await page.getByRole('button', {name: /사용자·권한/}).click(); await page.getByRole('heading', {name: '사용자·권한', exact: true}).waitFor(); await page.getByPlaceholder('이메일 검색').fill('fan@example.com'); await page.getByRole('button', {name: '검색'}).click(); await page.getByText('fan@example.com', {exact: true}).waitFor(); await page.getByRole('combobox', {name: '사용자 역할 필터'}).selectOption('fan'); await page.getByPlaceholder('이메일 검색').fill(''); await page.getByRole('combobox', {name: '사용자 역할 필터'}).selectOption('all'); await page.getByText('현재 세션').waitFor();"
