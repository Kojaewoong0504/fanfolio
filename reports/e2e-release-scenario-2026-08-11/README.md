...
## 2026-08-12 계약 검증 업데이트

배포 환경 수동 재실행은 수행하지 않았다. 대신 로컬 테스트 데이터베이스에서 결정적 계약 시나리오를 추가해 다음 흐름을 검증했다.

1. 아티스트가 스페셜 카드를 생성하고 회사 검수를 요청한다.
2. 회사 관리자가 회사 단계 검수를 승인한다.
3. 플랫폼 운영자가 스페셜 카드의 플랫폼 단계 검수를 승인한다.
4. 회사 관리자가 자기 조직 범위의 드롭을 만들고 승인된 카드를 연결한 뒤 라이브 상태로 전환한다.
5. 회사 관리자가 라이브 드롭에 연결된 카드의 코드 배치를 생성한다.
6. 팬이 QA 코드 1개를 QR 출처로 등록하면 카드 소유권, XP, 완료 업적, 수령 가능 보상이 생성된다.
7. 같은 코드 재등록은 `409 REDEEM_CODE_ALREADY_USED`이며 XP가 추가되지 않는다.
8. 같은 보상 수령 요청은 소유자에게 같은 결과를 반환해 멱등성을 유지한다.
9. 루트 관리자는 글로벌 드롭 정책 접근은 유지하지만 파트너 범위 드롭 생성, 드롭 연결, 라이브 전환, 코드 발급에는 참여할 수 없다.

관리자 대시보드에는 범위별 `growthSummary` 집계만 추가했다. 루트는 전체 합계, 회사 관리자는 자기 조직 합계만 보며, 팬 이메일·닉네임·사용자 ID·랭킹 목록은 응답하지 않는다.

보고서에는 계정 비밀번호, 세션 토큰, 액세스 토큰, 원본 QR 이미지, 실제 코드 문자열을 기록하지 않았다. 계약 테스트의 코드 값은 런타임에서만 조회하고 assertion에는 상태와 집계만 사용한다.
