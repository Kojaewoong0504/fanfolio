...
run_code "await page.getByRole('button', {name: '+ 카드 등록'}).click(); await page.getByRole('button', {name: /QR QR 스캔/}).click(); await page.getByText(/이 브라우저에서는 QR 스캔을 지원하지 않습니다|카메라를 사용할 수 없습니다/).waitFor(); await page.getByText('사진으로 QR 읽기').waitFor(); await page.getByPlaceholder('예: NOVA-VALID-01').fill('${REDEEM_CODE}'); await page.getByRole('button', {name: '카드 등록하기'}).click(); await page.getByRole('button', {name: '카드 공개하기'}).click(); await page.getByRole('heading', {name: '새 카드가 컬렉션에 추가됐어요'}).waitFor();"
