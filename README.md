...
개발 환경에서는 서버 시작 시 로컬 아티스트 스튜디오 점검 계정이 자동으로 보장됩니다.
아티스트 스튜디오 로그인은 `local-artist-studio` / `local-artist-password-2026`을 사용하세요.
이 계정은 개발 환경에서만 생성되며 운영·스테이징에는 자동 생성되지 않습니다.
관리자 웹도 개발 서버 시작 시 로컬 점검 계정이 자동으로 보장됩니다. 관리자 로그인은
`local-admin@example.com` / `local-admin-password-2026`을 사용하세요. 두 계정 모두 매번
브라우저 세션을 강제로 유지하는 방식이 아니라, 서버 시작 때 DB에 계정을 보장한 뒤 각
웹 앱에서 정상 로그인하는 방식입니다.
...
재시작하면 생성되며, 초기 관리자는 별도 강제 변경 화면 없이 바로 운영 화면에 들어갑니다.
초기 관리자는 관리자 웹의 사용자·권한 화면에서 다른 관리자 이메일을 등록할 수 있습니다.
발급된 관리자는 임시 비밀번호로 로그인한 뒤 본인 비밀번호로 변경합니다. 이메일 발송 연동 전에는
