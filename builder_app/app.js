...
async function logoutArtist() { try { await api('/auth/logout', { method: 'POST' }); } catch { /* 세션이 이미 만료된 경우에도 로컬 상태는 정리한다. */ } ACCESS_TOKEN = ''; state.authenticated = false; state.loginError = ''; render(); }
