...
run_code "await page.goto('http://localhost:4175'); await page.evaluate(() => localStorage.setItem('fanfolio_api_base', 'http://localhost:8000/api')); await page.reload(); await page.getByPlaceholder('artist@fanfolio.com').fill('artist@example.com'); await page.getByRole('button', {name: '로그인 링크 받기'}).click(); await page.getByPlaceholder('artist@fanfolio.com').waitFor({state: 'detached'}); await page.getByPlaceholder('이메일의 로그인 토큰').fill('test-magic-link-artist'); await page.getByRole('button', {name: '스튜디오 입장'}).click(); await page.getByRole('heading', {name: '카드 만들기'}).waitFor(); await page.getByText('● API 연결됨').waitFor();"
...
run_code "await page.getByRole('button', {name: '드림스케이프'}).click(); await page.getByRole('button', {name: '다음: 멤버 선택'}).click(); await page.getByRole('button', {name: '유나'}).click(); await page.getByRole('button', {name: '다음: 닉네임 설정'}).click(); await page.getByPlaceholder('닉네임을 입력하세요').fill('E2E팬'); await page.getByRole('button', {name: '나만의 컬렉션 시작하기'}).click();"
...
run_code "await page.goto('http://localhost:5173'); await page.route('**/api/auth/magic-link/request', async route => { await route.fulfill({status: 502, contentType: 'application/json', body: JSON.stringify({error: {message: 'API 요청에 실패했습니다. (502)'}})}); }); await page.getByPlaceholder('이메일을 입력하세요').fill('fan@example.com'); await page.getByRole('button', {name: '로그인 링크 받기'}).click(); await page.getByRole('alert').filter({hasText: 'API 요청에 실패했습니다. (502)'}).waitFor(); await page.unroute('**/api/auth/magic-link/request');"
