...
function studioCardsView() {
  ...
  const cards = state.cards.length ? state.cards.map((card) => `<article class="studio-card-row"><div><strong>${esc(card.name)}</strong><span>${esc(card.seasonName || '시즌 미정')} · ${esc(card.rarity || '타입 미정')}</span></div><span class="studio-status">${esc(statusLabels[card.status] || card.status)}</span><small>${card.issueLimit ? `${esc(card.issueLimit)}장` : '발행 수량 미정'}</small><button class="secondary card-edit" data-card-id="${esc(card.id)}">수정</button></article>`).join('') : '<div class="studio-empty"><strong>아직 만든 카드가 없어요.</strong><span>첫 번째 특별 카드를 만들어 보세요.</span></div>';
...
document.addEventListener('submit', async (event) => {
  ...
  state.form = { ...state.form, name: form.get('name'), memberId: form.get('memberId'), seasonName: form.get('seasonName'), templateId: form.get('templateId'), rarity: form.get('rarity'), signatureText: form.get('signatureText'), issueLimit: Number(form.get('issueLimit')) };
  try {
    ...
    const result = await api(`/artist/cards/${state.editingCardId}`, { method: 'PATCH', body: JSON.stringify({ templateId: state.form.templateId, name: state.form.name, seasonName: state.form.seasonName, rarity: state.form.rarity, imageAssetId, memberId: state.form.memberId, signatureText: state.form.signatureText, hasVoice: state.form.hasVoice, issueLimit: state.form.issueLimit }) });
