...
async function uploadAsset(file, purpose) { const presigned = await api('/uploads/presign', { method: 'POST', body: JSON.stringify({ fileName: file.name, contentType: file.type, purpose }) }); const directUpload = presigned.data.uploadMode === 'direct'; const upload = await fetch(absoluteApiUrl(presigned.data.uploadUrl), { method: 'PUT', body: file, credentials: directUpload ? 'omit' : 'include', headers: { 'Content-Type': file.type, ...(directUpload ? {} : { 'X-Fanfolio-Client': 'artist', ...(SESSION_TOKEN ? { 'X-Fanfolio-Session': SESSION_TOKEN } : {}) }) } }); if (!upload.ok) throw new Error(`UPLOAD ${upload.status}`); if (presigned.data.completeUrl) await api(presigned.data.completeUrl.replace(/^\/api/, ''), { method: 'POST' }); return presigned.data.assetId; }
