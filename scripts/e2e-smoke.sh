...
run_code "await page.getByRole('button', {name: /드롭·코드 관리/}).click(); await page.locator('select[name=cardId]').waitFor(); await page.locator('select[name=cardId]').selectOption({label: 'E2E 공식 특별 카드'}); await page.locator('input[name=quantity]').fill('1'); await page.locator('input[name=maxUsesPerCode]').fill('1'); await page.locator('input[name=expiresAt]').fill('2030-12-31T23:59'); await page.locator('input[name=prefix]').fill('E2E'); await page.getByRole('button', {name: '코드 배치 생성'}).click(); await page.getByText(/배치 batch_/).waitFor(); await page.getByRole('heading', {name: '생성된 코드 배치'}).waitFor(); await page.getByRole('row', {name: /batch_/}).waitFor();"
