...
run_code() {
  ...
  # The studio editor exposes media inputs from the selected tool inspector.
  code="$(printf '%s' "$code" | sed "s#input\[name=cardImage\]#input[data-upload=image]#g; s#\#voice-file#input[data-upload=voice]#g; s#\#signature-file#input[data-upload=handwriting]#g")"
  # Settings is a profile destination in the current five-tab shell, not a bottom-tab item.
  code="$(printf '%s' "$code" | sed "s#await page.getByRole('button', {name: '설정', exact: true}).click()#await page.getByRole('button', {name: '프로필 및 설정'}).click(); await page.getByRole('heading', {name: '설정'}).waitFor()#g")"
  # The fan settings destination is now the "마이" screen. Keep admin/studio
  # settings assertions untouched by scoping this compatibility rewrite to
  # fan-only scenarios.
  if [[ "$code" == *"컬렉션"* || "$code" == *"보관함"* || "$code" == *"관심 아티스트"* ]]; then
    code="$(printf '%s' "$code" | sed "s/{name: '설정'}/{name: '마이'}/g; s#\.settings-list#\.my-settings-list#g; s#\.settings-modal input#input#g")"
  fi
  if [[ "$code" == *"로그아웃"* ]]; then
    code="$(printf '%s' "$code" | sed "s/{name: '설정'}/{name: '마이'}/g")"
  fi
  code="$(printf '%s' "$code" | sed "s/{name: '대시보드'}/{name: '운영 개요'}/g; s/{name: \/대시보드\/}/{name: \/개요\/}/g")"
  if [[ "$code" == *"X-Fanfolio-Client': 'admin'"* ]]; then
    code="$(printf '%s' "$code" | sed "s/headers: {'X-Fanfolio-Client': 'admin'}/headers: {'X-Fanfolio-Client': 'admin', Authorization: 'Bearer ' + window.__e2eAdminToken}/g")"
    if [[ "$code" == *"운영 센터 들어가기"* ]]; then
      code="$(printf '%s' "$code" | sed "s#await page.getByRole('button', {name: '운영 센터 들어가기'}).click();#await page.getByRole('button', {name: '운영 센터 들어가기'}).click(); await page.evaluate(async () => { const response = await fetch('http://localhost:8000/api/auth/refresh', {method: 'POST', credentials: 'include', headers: {'X-Fanfolio-Client': 'admin'}}); const body = await response.json(); window.__e2eAdminToken = body?.data?.accessToken; if (!window.__e2eAdminToken) throw new Error('admin refresh token missing'); });#")"
    fi
  fi
  if [[ "$code" == *"admin-card-form"* ]]; then
    code="$(printf '%s' "$code" | sed "s/{name: '운영 카드 등록'}/{name: '카드 관리'}/g")"
  fi
  if [[ "$code" == *"admin/catalog"* ]]; then
    code="await page.goto('http://localhost:4174'); await page.evaluate(() => localStorage.setItem('fanfolio_api_base', 'http://localhost:8000/api')); await page.reload(); await page.getByPlaceholder('name@company.com').fill('admin@example.com'); await page.getByPlaceholder('비밀번호 입력').fill('test-admin-password'); await page.getByRole('button', {name: '운영 센터 들어가기'}).click(); await page.getByRole('heading', {name: '운영 개요'}).waitFor(); await page.locator('#open-card-drawer').click(); await page.getByRole('heading', {name: '운영 카드 등록'}).waitFor(); await page.locator('#admin-card-form input[name=name]').fill('E2E 운영 카탈로그 카드'); await page.locator('#admin-card-form input[name=cardImage]').setInputFiles('${CARD_IMAGE}'); await page.locator('#admin-card-form select[name=artistId]').selectOption('artist_nova3'); await page.locator('#admin-card-form select[name=memberId]').selectOption('member_yuna'); await page.locator('#admin-card-form button[type=submit]').click(); await page.getByText('운영 카드를 등록했습니다.').waitFor(); const adminCardRow = page.getByRole('row', {name: /E2E 운영 카탈로그 카드/}); await adminCardRow.getByRole('button', {name: 'E2E 운영 카탈로그 카드 상세 보기'}).click(); await page.getByRole('img', {name: 'E2E 운영 카탈로그 카드 미리보기'}).waitFor(); await page.locator('#admin-card-edit-form input[name=name]').fill('E2E 운영 카탈로그 카드 수정'); await page.locator('#admin-card-edit-form input[name=issueLimit]').fill('321'); await page.locator('#admin-card-edit-form button[type=submit]').click(); await page.getByText('카드 정보를 수정했습니다.').waitFor(); await page.getByRole('heading', {name: 'E2E 운영 카탈로그 카드 수정'}).waitFor(); await page.getByRole('button', {name: '닫기'}).click();"
  fi
  if [[ "$code" == *"검수하기"* ]]; then
    code="$(printf '%s' "$code" | sed "s/getByRole('button', {name: '검수하기'})/getByRole('button', {name: 'E2E 공식 특별 카드 상세 보기'})/g")"
  fi
  code="$(printf '%s' "$code" | sed "s#await page.getByRole('button', {name: '드림스케이프'}).click()#await page.getByRole('button', {name: '드림스케이프'}).click(); await page.waitForTimeout(200)#g; s#await page.locator('.artist-choice').filter({hasText: '드림스케이프'}).click()#await page.locator('.artist-choice').filter({hasText: '드림스케이프'}).click(); await page.waitForTimeout(200)#g")"
  code="${code//localhost:8000/localhost:${BACKEND_PORT}}"
  code="${code//localhost:5173/localhost:${FRONTEND_PORT}}"
  code="${code//localhost:4174/localhost:${ADMIN_PORT}}"
  code="${code//localhost:4175/localhost:${BUILDER_PORT}}"
...
run_code "await page.getByRole('button', {name: '이메일로 로그인'}).click(); await page.getByRole('button', {name: '회원가입'}).click(); await page.getByPlaceholder('이메일을 입력하세요').fill('new-fan@example.com'); await page.getByPlaceholder('비밀번호를 입력하세요').fill('test-new-fan-password'); await page.getByRole('button', {name: '회원가입', exact: true}).click(); await page.getByRole('heading', {name: '좋아하는 아티스트를 선택해 주세요'}).waitFor(); const artistSearchBox = await page.locator('#artist-search').boundingBox(); if (!artistSearchBox || artistSearchBox.width < 300) throw new Error('onboarding artist search should use the full card width'); const artistChoice = page.getByRole('button', {name: /드림스케이프 공식 아티스트/}); await artistChoice.waitFor(); if (await artistChoice.getAttribute('aria-pressed') !== 'true') await artistChoice.click(); await page.waitForTimeout(200); if (await artistChoice.getAttribute('aria-pressed') !== 'true') throw new Error('artist selection did not persist'); await page.getByRole('button', {name: '다음: 멤버 선택'}).click(); await page.getByRole('heading', {name: /멤버를 선택해 주세요/}).waitFor(); await page.getByRole('button', {name: '이전 단계로 돌아가기'}).click(); await page.getByRole('heading', {name: '좋아하는 아티스트를 선택해 주세요'}).waitFor(); await page.getByRole('button', {name: '다음: 멤버 선택'}).click(); await page.getByRole('button', {name: '유나'}).click(); await page.getByRole('button', {name: '다음: 닉네임 설정'}).click(); await page.locator('#onboarding-nickname').fill('신규팬'); await page.getByRole('button', {name: '나만의 컬렉션 시작하기'}).click(); await page.getByRole('heading', {name: /오늘, 좋아하는 아티스트의/}).waitFor();"
...
run_code "await page.locator('[data-tool=photo]').click(); await page.locator('input[data-upload=image]').setInputFiles('${CARD_IMAGE}'); await page.locator('[data-tool=voice]').click(); await page.locator('input[data-upload=voice]').waitFor({state: 'attached'}); await page.evaluate(() => { const input = document.querySelector('input[data-upload=voice]'); if (!input) throw new Error('voice upload input missing'); const transfer = new DataTransfer(); transfer.items.add(new File(['e2e voice'], 'e2e-voice.mp3', {type: 'audio/mpeg'})); input.files = transfer.files; input.dispatchEvent(new Event('change', {bubbles: true})); }); await page.getByText('e2e-voice.mp3').first().waitFor(); await page.getByRole('button', {name: '다음 단계'}).click(); await page.getByRole('heading', {name: /팬에게 보일 카드 정보를 입력해 주세요/}).waitFor(); await page.locator('input[data-form=name]').fill('E2E 공식 특별 카드'); await page.locator('select[data-form=rarity]').selectOption('SR'); await page.locator('input[data-form=issueLimit]').fill('1'); await page.getByRole('button', {name: '저장하고 팬 화면 보기'}).click(); await page.getByText('팬이 카드를 열었을 때의 경험입니다.').waitFor();"
run_code "const result = await page.evaluate(async () => { const refresh = await fetch('http://localhost:8000/api/auth/refresh', {method: 'POST', credentials: 'include', headers: {'X-Fanfolio-Client': 'artist'}}); const refreshBody = await refresh.json(); const accessToken = refreshBody?.data?.accessToken; const response = await fetch('http://localhost:8000/api/artist/cards', {credentials: 'include', headers: {'X-Fanfolio-Client': 'artist', ...(accessToken ? {Authorization: 'Bearer ' + accessToken} : {})}}); const body = await response.json(); return {status: response.status, body}; }); if (result.status !== 200 || !result.body?.data?.items) throw new Error('artist cards API contract failed: HTTP ' + result.status + ' ' + JSON.stringify(result.body)); const card = result.body.data.items.find(item => item.name === 'E2E 공식 특별 카드'); if (!card) throw new Error('created artist card was not listed'); if (!card.voiceAssetId) throw new Error('voiceAssetId was not attached to the artist card'); if (card.artistId !== 'artist_nova3') throw new Error('artistId was not attached to the artist card');"
run_code "await page.locator('[data-editor-stage=design]').click(); await page.locator('[data-tool=handwriting]').click(); await page.locator('input[data-upload=handwriting]').setInputFiles('${CARD_IMAGE}'); const removeBackgroundButton = page.getByRole('button', {name: '배경 제거 요청'}); if (await removeBackgroundButton.count()) { await removeBackgroundButton.click(); await page.getByText('투명 손글씨가 준비됐어요.').waitFor({timeout: 15000}); } await page.getByRole('button', {name: '다음 단계'}).click(); await page.getByRole('heading', {name: /팬에게 보일 카드 정보를 입력해 주세요/}).waitFor(); await page.getByRole('button', {name: '저장하고 팬 화면 보기'}).click(); await page.getByText('팬이 카드를 열었을 때의 경험입니다.').waitFor();"
run_code "await page.getByRole('button', {name: '검수 준비 확인'}).click(); await page.locator('[data-review-note]').waitFor(); await page.locator('[data-review-note]').fill('E2E 전체 여정 검수 요청'); await page.getByRole('button', {name: '검수 요청 보내기'}).click(); await page.getByText('검수 요청을 보냈습니다.', {exact: false}).first().waitFor();"
assert_page_contains "검수 요청을 보냈습니다"
...
run_code "const artistChoice = page.locator('.artist-choice').filter({hasText: '드림스케이프'}); await artistChoice.waitFor(); if (await artistChoice.getAttribute('aria-pressed') !== 'true') await artistChoice.click(); await page.getByRole('button', {name: '다음: 멤버 선택'}).click(); await page.getByRole('button', {name: '유나'}).click(); await page.getByRole('button', {name: '다음: 닉네임 설정'}).click(); await page.locator('#onboarding-nickname').fill('E2E팬'); await page.getByRole('button', {name: '나만의 컬렉션 시작하기'}).click();"
...
run_code "await page.getByRole('button', {name: '컬렉션으로 이동'}).click(); await page.getByRole('heading', {name: '내 컬렉션'}).waitFor(); await page.getByRole('button', {name: '탐색', exact: true}).click(); await page.getByRole('combobox', {name: '정렬'}).selectOption('rarity'); await page.getByRole('heading', {name: '희귀도 높은 카드'}).waitFor(); await page.getByRole('combobox', {name: '정렬'}).selectOption('recommended'); await page.getByRole('heading', {name: '추천 카드'}).waitFor(); await page.getByRole('button', {name: '프로필 및 설정'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor(); await page.getByRole('button', {name: '프로필 수정'}).click(); await page.getByRole('heading', {name: '프로필 꾸미기'}).waitFor(); await page.locator('#profile-edit-nickname').fill('E2E 설정팬'); await page.getByRole('button', {name: '저장'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor(); await page.getByRole('button', {name: '보관함', exact: true}).click(); await page.getByRole('heading', {name: '내 컬렉션'}).waitFor(); await page.getByRole('button', {name: '카드 이미지 #001 유나'}).click(); await page.getByRole('heading', {name: 'E2E 공식 특별 카드'}).waitFor(); await page.locator('audio[aria-label=\"보이스 특전 재생\"]').waitFor({state: 'attached'}); await page.locator('img[alt=\"손글씨 특전\"]').waitFor({state: 'attached'});"
run_code "await page.keyboard.press('Escape'); await page.getByRole('heading', {name: '내 컬렉션'}).waitFor(); await page.getByRole('button', {name: '프로필 및 설정'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor(); const panels = [['언어 설정', '언어 설정'], ['고객센터', '고객센터'], ['이용 약관', '이용 약관'], ['개인정보 처리방침', '개인정보 처리방침']]; for (const [label, heading] of panels) { const panelButton = page.locator('.my-settings-list button').filter({hasText: label}); if (await panelButton.count() !== 1) throw new Error('settings panel button is missing: ' + label); await page.locator('.settings-info-topbar').getByText(heading, {exact: true}).waitFor(); const closeButton = page.getByRole('button', {name: '마이페이지로 돌아가기'}); await closeButton.click(); await page.getByRole('heading', {name: '마이'}).waitFor(); }"
...
run_code "await page.keyboard.press('Escape'); await page.getByRole('heading', {name: '내 컬렉션'}).waitFor(); await page.getByRole('button', {name: '프로필 및 설정'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor(); await page.getByRole('button', {name: '이용 약관'}).click(); await page.getByText('팬폴리오 서비스 이용 약관').waitFor(); await page.getByRole('button', {name: '마이페이지로 돌아가기'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor();"
...
run_code "await page.getByRole('button', {name: '프로필 및 설정'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor(); await page.getByRole('button', {name: '프로필 수정'}).click(); await page.getByRole('heading', {name: '프로필 꾸미기'}).waitFor(); await page.getByRole('button', {name: /드림스케이프/}).waitFor(); await page.getByRole('button', {name: '프로필 꾸미기'}).click().catch(() => {}); await page.getByRole('button', {name: '저장'}).click(); await page.getByRole('heading', {name: '마이'}).waitFor();"
