...
run_code "await page.goto('http://localhost:4174'); await page.getByRole('heading', {name: '대시보드'}).waitFor(); await page.goto('http://localhost:4175'); await page.getByRole('heading', {name: '카드 만들기'}).waitFor(); await page.getByRole('button', {name: '내 카드'}).click(); await page.getByText('E2E 공식 특별 카드').waitFor(); const publishedRow = page.locator('.studio-card-row').filter({hasText: 'E2E 공식 특별 카드'}); if (await publishedRow.locator('button.card-edit').count() !== 0) throw new Error('published cards must be read-only in the artist studio'); await page.getByRole('button', {name: '팬 반응'}).click(); await page.getByRole('heading', {name: '팬 반응'}).waitFor(); await page.getByText('전체 수집 수').waitFor(); await page.getByText('E2E 공식 특별 카드').waitFor(); await page.goto('http://localhost:5173'); await page.getByRole('heading', {name: '내 컬렉션'}).waitFor(); await page.getByRole('button', {name: '카드 이미지 #001 유나'}).click(); await page.getByRole('heading', {name: 'E2E 공식 특별 카드'}).waitFor();"
assert_page_contains "E2E 공식 특별 카드"
