...
run_code "await page.getByRole('button', {name: '+ 카드 등록'}).click(); await page.getByRole('button', {name: /QR QR 스캔/}).click(); await page.getByText(/이 브라우저에서는 QR 스캔을 지원하지 않습니다|카메라를 사용할 수 없습니다/).waitFor(); await page.getByText('사진으로 QR 읽기').waitFor(); await page.locator('.qr-photo-upload input').setInputFiles('${QR_IMAGE}'); await page.getByText('사진에서 QR 코드가 인식되었습니다.').waitFor(); await page.getByRole('button', {name: '카드 등록하기'}).click(); await page.getByRole('button', {name: '카드 공개하기'}).click(); await page.getByRole('heading', {name: '새 카드가 컬렉션에 추가됐어요'}).waitFor();"
assert_page_contains "E2E 공식 특별 카드"
run_code "await page.getByRole('button', {name: '컬렉션으로 이동'}).click(); await page.getByRole('button', {name: '카드 이미지 #001 유나'}).click(); await page.getByRole('heading', {name: 'E2E 공식 특별 카드'}).waitFor(); await page.locator('audio[aria-label=\"보이스 특전 재생\"]').waitFor({state: 'attached'}); await page.locator('img[alt=\"손글씨 특전\"]').waitFor({state: 'attached'});"
