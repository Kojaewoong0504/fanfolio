...
run_code "await page.locator('#campaign-form input[name=name]').fill('E2E 특전 캠페인'); await page.locator('#campaign-form select[name=artistId]').selectOption('artist_nova3'); await page.locator('#campaign-form select[name=requiredCardIds]').selectOption('card_published'); await page.locator('#campaign-form input[name=benefitTitle]').fill('E2E 디지털 특전'); await page.locator('#campaign-form input[name=benefitDescription]').fill('E2E 완성 특전'); await page.locator('#campaign-form button[type=submit]').click(); await page.getByText('특전 캠페인을 등록했습니다.').waitFor(); await page.getByText('E2E 특전 캠페인').waitFor();"
