...
run_code "const result = await page.evaluate(async () => { const benefits = await (await fetch('/api/me/collection/benefits', {credentials: 'include'})).json(); const target = benefits.data.items.find(item => item.claimable); if (!target) throw new Error('completed benefit was not claimable'); const claim = await fetch('/api/me/collection/benefits/' + target.campaignId + '/claim', {method: 'POST', credentials: 'include'}); if (claim.status !== 201) throw new Error('benefit claim failed: ' + claim.status); const duplicate = await fetch('/api/me/collection/benefits/' + target.campaignId + '/claim', {method: 'POST', credentials: 'include'}); if (duplicate.status !== 409) throw new Error('duplicate benefit claim was accepted'); return claim.json(); }); if (!result.data?.claimId) throw new Error('benefit claim response did not include a claim id');"
