...
export function FanPublicProfile({ userId, onBack, onOpenArtist, onOpenCollection, onTrade, initialCollection }: Props) {
  ...
  const toggleFollow = async () => { if (initialCollection) { applyFollowing(!following); return } try { const r = following ? await unfollowFan(userId) : await followFan(userId); applyFollowing(r.data.following) } catch { setError('팔로우 상태를 변경하지 못했어요.') } }
